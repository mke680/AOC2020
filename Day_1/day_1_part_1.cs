...
namespace AOC
{
    public class Day1_Part1
    {
        ...
        {
            using(var reader = new StreamReader(@"C:\Users\mcho\OneDrive - GRACE WORLDWIDE (AUSTRALIA) PTY LTD\Documents\AOC\Day_1\Day1Test.txt"))
            {
                    ...
                    if(entry1 > 1010){
                        foreach(int entry2 in listEntries){
                            if(entry1 + entry2 == 2020){
                                Console.WriteLine(entry1 * entry2);return;
                            }
                        }
                    }
                }
